// Torch-shim header for the torch-free libgemm_capi.so build.
//
...
// CUDA error reporting. When building libgemm_capi.so we resolve
...
#define TORCH_CHECK(cond, ...)                                                \
  do {                                                                        \
    if (!(cond)) {                                                            \
      ::capi_torch_shim::torch_check_fail(__VA_ARGS__);                 \
    }                                                                         \
  } while (0)
#endif
